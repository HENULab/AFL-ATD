...
logger.add(f"./log/{current_time}.log")
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr":  # convex
            ...
            if "custom" in args.dataset:
                args.model = VGG(num_classes=args.num_classes).to(args.device)
    ...
    logger.info(f"Average time cost: {round(np.average(time_list), 2)}s.")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-data', "--dataset", type=str, default="custom")
    parser.add_argument('-nb', "--num_classes", type=int, default=2)
    parser.add_argument('-m', "--model", type=str, default="cnn")
    parser.add_argument('-lbs', "--batch_size", type=int, default=10)
    parser.add_argument('-lr', "--local_learning_rate", type=float, default=0.007,
    ...
    parser.add_argument('-lam', "--lamda", type=float, default=5.0,
    ...
    if args.device == "cuda" and not torch.cuda.is_available():
        logger.info("cuda is not avaiable.")
        args.device = "cpu"
    ...
    logger.info("Fine tuning epoches on new clients: {}".format(args.fine_tuning_epoch))
    logger.info("=" * 50)
    ...
    # logger.info(f"Total time cost: {round(time.time()-total_start, 2)}s.")
